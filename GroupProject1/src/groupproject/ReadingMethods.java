...
public class ReadingMethods 
{
       ...
       //Not sure about throwing the FileNoFoundException might not be best way to do this.
        ...
        public ReadingMethods(String landscapeFile) throws FileNotFoundException
        {
              ...
              BufferedReader fileName = new BufferedReader(new FileReader(landscapeFile));
              Scanner landscapeScanner = new Scanner(fileName);
       ...
       // System.out.println(percentPuma);
        ...
        int rows = landscape.getNRows();
        int cols = landscape.getNCols();
        
                    
             for(int k=0; k < rows; k++) {
                for(int l =0; l < cols; l++){
                    if(landscape.isDry(k,l)){
                        population.setSquarePop("predator", random.nextInt((int)percentPuma), k, l);
                    }
                }
            }                       
            ...
            for(int k=0; k < rows; k++) {
                for(int l =0; l < cols; l++) {
                    if(landscape.isDry(k,l)){
                        population.setSquarePop("prey", random.nextInt((int)percentHare), k, l);
                    }
                }
            }
         
         
         double pumaPop = population.getPredatorAverageDensity();
         double harePop = population.getPreyAverageDensity();
