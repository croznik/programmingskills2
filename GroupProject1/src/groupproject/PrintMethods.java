  ...
  */
  ...
  public static void printDensityFile(PrintWriter outfile, Population p){
     ...
     double preyDens = p.getPreyAverageDensity();
      //double predatorDens = p.getTotalPop(p.getPredatorMap());
      //double preyDens = p.getTotalPop(p.getPreyMap());
     ...
     // outfile2.println(+p.getTime()+" "+preyDens);
         ...
         //Loop over rows
           for(int k=0; k<map.length;k++)
           {
              //If there is no population in a square its rgb value is now set to 255, 255, 255 (white)
                ...
                int value = PrintMethods.convertDensityToRGB(map[k][i], total);
                outMap[k][(3*i)+colorIndex] = value;
